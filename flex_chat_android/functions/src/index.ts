...
import fcmCredential from "./flex-chat-d996e-firebase-adminsdk-haa8u-2ff4478b68.json";

admin.initializeApp({
  credential: admin.credential.cert({
    privateKey: fcmCredential.private_key,
    projectId: fcmCredential.project_id,
    clientEmail: fcmCredential.client_email,
  }),
});
...
export const messageNotification = onDocumentCreated(
  ...
  async (snapshot) => {
    ...
    try {
      const isSentResult = await admin.messaging().sendToTopic(message.conversationId, {
        data: {
